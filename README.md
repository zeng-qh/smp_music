...
> **myMPD 不在 Debian 官方源里**（`packages.debian.org` 全套件搜索零结果），
> 镜像里是从 myMPD 官方的 openSUSE OBS 仓库装的。同理，
> 蓝牙守护进程的包名是 `bluez-alsa-utils`，不是 `bluealsa`。
...
> 容器内部分工：`myMPD` 监听 `127.0.0.1:8080`，`nginx` 占用 80 做登录鉴权后反代到它，`MPD` 的 HTTP 流在 8001 由 nginx 8000 转发。
> 注意 **myMPD 默认就是 80 端口**，会和 nginx 直接冲突，脚本会在启动时强制改成 8080。
...
```bash
# 注意：Debian 里的包名是 bluez-alsa-utils，不是 bluealsa（后者不存在）
...
镜像内置一段 15 秒 440Hz 正弦波。曲库空的时候用它验证每一路输出是否真的出声。

它实际存放在镜像的 `/opt/music/testtone/`，容器启动时才软链进你的音乐目录 —— 因为音乐目录通常是宿主机挂载进来的，直接放在 `/music` 会被挂载遮蔽掉。
...
```bash
docker compose logs music          # 三个进程的日志都在这里
docker compose exec music mpc outputs
```

| 现象 | 原因 / 处理 |
|---|---|
| 网页 502 | myMPD 没起来。看日志里是否有 `Unable to connect to ...:80` —— 那说明 myMPD 仍是默认 80，和 nginx 撞了。确认 `/var/lib/mympd/config/http_port` 内容为 `8080` |
| 网页能开但曲库为空 | 音乐目录没挂对，或权限不足。`docker compose exec music ls /music` 看得到文件才行；再 `mpc update` |
| 蓝牙通道点了没声音 | 宿主机没配对成功。`bluetoothctl info <MAC>` 里要显示 `Connected: yes` |
| myMPD 报权限错误 | `/var/lib/mympd` 是挂载卷，宿主目录归属可能不对，进容器 `chown -R root:root /var/lib/mympd` |
